    ...
    <div class="row justify-center q-mt-xl">
      <div class="col-sm-12 col-md-10 col-lg-8 col-xl-6">
        ...
        <q-item>
          ...
          <!-- Email & Nickname -->
          <q-item-section>
            <q-item-label lines="1" class="text-body1">{{ userInfo.email }}</q-item-label>
            <q-item-label>
              <q-input v-if="!editMode" v-model="nickname" :default="userInfo.nickname" label="Nickname" />
              <q-item-label v-else class="text-body1 q-py-md">{{ userInfo.nickname }}</q-item-label>
            </q-item-label>
          </q-item-section>
        </q-item>

        <q-list class="q-mt-lg">
          <q-item class="row" v-for="(idx, item) in getUserCategories.data" :key="item">
              <q-item-section class="col-2">
                <q-item-label lines="1">{{ item }}</q-item-label>
              </q-item-section>

              <q-item-section class="col-10">
                <q-linear-progress :value="getUserCategories.data[item] / getUserCategories.cnt * 3"
                  :color="colorRange[Object.keys(getUserCategories.data).indexOf(item)]" class="q-mt-sm" />
              </q-item-section>
          </q-item>
        </q-list>
        ...
        <q-item class="row justify-center q-mt-lg">
          <div v-if="editMode" class="q-mx-xs">
            <q-btn @click="changeDeleteMode" color="negative" label="회원 탈퇴" />
          </div>

          <div v-if="editMode" class="q-mx-xs">
            <q-btn @click="changeEditMode" padding="5px 30px" color="warning" label="수정" />
          </div>
          <div v-else class="q-mx-xs">
            <q-btn @click="updateUserInfo" padding="5px 30px" color="warning" label="완료" />
          </div>
        ...
        <q-dialog v-model="deleteMode" persistent>
          <q-card>
            <q-card-section class="row items-center">
              <span class="q-mx-md q-mt-md">정말로 회원 탈퇴를 진행하시겠습니까?</span>
            </q-card-section>

            <q-card-actions align="right">
              <q-btn flat @click="deleteUser" label="회원 탈퇴" color="negative" v-close-popup />
