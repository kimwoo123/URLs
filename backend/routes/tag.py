...
@tag.post('/tag', summary="새로운 태그 생성")
async def create_tag(tag: Tag):
    db.tag.insert_one(dict(tag))
    return serializeList(db.tag.find())


@tag.put('/tag/{tag_name}', summary="태그 정보 수정")
...
# 체크
@tag.post('/tag/{tag_name}', summary="태그에 새로운 url 추가 및 삭제")
...
@tag.delete('/tag/{tag_name}', summary="태그 삭제")
